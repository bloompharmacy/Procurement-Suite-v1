...
GPO.App = (function () {
  ...
  function route(name) {
    ...
    var r = { dashboard: renderDashboard, suppliers: renderSuppliers, sellout: renderSellout,
      ...
      offers: renderOffers, budgets: renderBudgets, importer: renderImporter,
      engine: renderEngine, users: renderUsers }[name];
    if (r) r();
  }
  ...
  function renderDashboard() {
    ...
    v.innerHTML =
      ...
      '<div class="card"><div class="row">' +
        ...
        '<div class="f"><label>Data horizon</label><select id="dHorizon" title="How much history to load from Firestore">' +
          ['12', '24', '36', '0'].map(function (h) { return opt(h, h === '0' ? 'All history' : 'Last ' + h + ' months', dashState.horizon || '24'); }).join('') + '</select></div>' +
      '</div><p class="muted" style="margin:8px 2px 0">Horizon controls how much sellout/orders history is fetched from the cloud (keeps reads proportional to what you view). Filters above narrow the loaded set for display.</p></div>' +
      '<div class="grid g4">' +
    ...
    // Data horizon -> scope the Firestore subscription (performance control).
    $('dHorizon').addEventListener('change', function (e) {
      dashState.horizon = e.target.value;
      var n = parseInt(e.target.value, 10);
      if (!n) { GPO.Store.setWindow(null, null); }
      else {
        var d = new Date();
        var wf = new Date(d.getFullYear(), d.getMonth() - n, 1);
        var wt = new Date(d.getFullYear(), d.getMonth() + 6, 1);
        var fmt = function (x) { return x.getFullYear() + '-' + String(x.getMonth() + 1).padStart(2, '0'); };
        GPO.Store.setWindow(fmt(wf), fmt(wt));
      }
      toast('Data horizon: ' + (n ? 'last ' + n + ' months' : 'all history'));
    });
  ...
  // ================================================================
  // MODULE: Data Importer (CSV / XLSX → gpo_db)
  // ================================================================
  var impState = { type: 'suppliers', parsed: null, map: {} };

  function renderImporter() {
    var v = $('view-importer');
    var t = impState.type;
    var typeTabs = [['suppliers', '🏢 Suppliers'], ['sellout', '📈 Sellout'], ['orders', '🧾 Purchase Orders']]
      .map(function (x) { return '<button class="act ' + (t === x[0] ? '' : 'ghost') + ' sm" onclick="GPO.App.impSetType(\'' + x[0] + '\')">' + x[1] + '</button>'; }).join(' ');

    v.innerHTML =
      '<h2 class="page">Universal Data Importer</h2>' +
      '<p class="page-sub">Upload CSV or Excel to populate the cloud database. Columns are auto-mapped; confirm and import. Data flows straight into forecasting, margins and the dashboard.</p>' +
      '<div class="card"><h3>1 · Choose data type</h3><div style="display:flex;gap:8px;flex-wrap:wrap">' + typeTabs + '</div>' +
        '<p class="muted" style="margin-top:10px">Need the format? <a href="#" onclick="GPO.App.impTemplate();return false;">Download the ' + t + ' CSV template</a>.</p></div>' +
      '<div class="card"><h3>2 · Upload file</h3>' +
        '<input type="file" id="impFile" accept=".csv,.xlsx,.xls">' +
        '<div id="impParseOut" style="margin-top:10px"></div></div>' +
      '<div id="impMapArea"></div>';

    $('impFile').addEventListener('change', function () {
      var file = this.files[0]; if (!file) return;
      GPO.Importer.parseFile(file).then(function (parsed) {
        impState.parsed = parsed;
        impState.map = GPO.Importer.autoMap(impState.type, parsed.headers);
        $('impParseOut').innerHTML = '<div class="note">Parsed <b>' + parsed.rows.length + '</b> rows, ' + parsed.headers.length + ' columns. Confirm the mapping below.</div>';
        renderImpMapping();
      }).catch(function (e) { $('impParseOut').innerHTML = '<div class="note" style="border-left-color:var(--bad)">⚠ ' + esc(e.message) + '</div>'; });
    });

    if (impState.parsed) renderImpMapping();
  }

  function renderImpMapping() {
    var area = $('impMapArea'); if (!area || !impState.parsed) return;
    var fields = GPO.Importer.FIELDS[impState.type];
    var headers = impState.parsed.headers;
    var optsHtml = '<option value="">— none —</option>' + headers.map(function (h) { return '<option value="' + esc(h) + '">' + esc(h) + '</option>'; }).join('');

    var mapRows = fields.map(function (f) {
      var sel = impState.map[f.key] || '';
      var selectHtml = '<select data-fk="' + f.key + '" class="impMapSel">' +
        optsHtml.replace('value="' + esc(sel) + '"', 'value="' + esc(sel) + '" selected') + '</select>';
      return '<tr><td>' + esc(f.label) + (f.required ? ' <span class="pill bad">required</span>' : '') + '</td><td>' + selectHtml + '</td></tr>';
    }).join('');

    // Preview first 5 rows
    var prevCols = fields.filter(function (f) { return impState.map[f.key]; });
    var prev = impState.parsed.rows.slice(0, 5).map(function (row) {
      return '<tr>' + prevCols.map(function (f) { return '<td>' + esc(row[impState.map[f.key]]) + '</td>'; }).join('') + '</tr>';
    }).join('');

    area.innerHTML =
      '<div class="card"><h3>3 · Confirm column mapping</h3>' +
        '<div class="tbl-wrap"><table><thead><tr><th>Target field</th><th>File column</th></tr></thead><tbody>' + mapRows + '</tbody></table></div>' +
        '<div style="margin-top:14px"><button class="act" id="impRun">Import ' + impState.parsed.rows.length + ' rows →</button></div>' +
        '<div id="impResult" style="margin-top:12px"></div></div>' +
      (prevCols.length ? '<div class="card"><h3>Preview (first 5 rows)</h3><div class="tbl-wrap"><table><thead><tr>' +
        prevCols.map(function (f) { return '<th>' + esc(f.label) + '</th>'; }).join('') + '</tr></thead><tbody>' + prev + '</tbody></table></div></div>' : '');

    area.querySelectorAll('.impMapSel').forEach(function (sel) {
      sel.addEventListener('change', function () {
        if (this.value) impState.map[this.dataset.fk] = this.value; else delete impState.map[this.dataset.fk];
        renderImpMapping();
      });
    });
    $('impRun').addEventListener('click', function () {
      var missing = GPO.Importer.FIELDS[impState.type].filter(function (f) { return f.required && !impState.map[f.key]; });
      if (missing.length) { toast('Map required: ' + missing.map(function (f) { return f.label; }).join(', ')); return; }
      try {
        var res = GPO.Importer.importRows(impState.type, impState.parsed.rows, impState.map);
        $('impResult').innerHTML = '<div class="note">✅ Imported <b>' + res.imported + '</b> ' + res.type +
          (res.groups ? ' rows across ' + res.groups + ' supplier-month group(s)' : ' record(s)') +
          (res.skipped ? ' · skipped ' + res.skipped + ' (missing required cells)' : '') + '. Now open Forecast, Margins or the Dashboard.</div>';
        toast('Imported ' + res.imported + ' ' + res.type);
      } catch (e) { $('impResult').innerHTML = '<div class="note" style="border-left-color:var(--bad)">⚠ ' + esc(e.message) + '</div>'; }
    });
  }
  function impSetType(t) { impState.type = t; impState.parsed = null; impState.map = {}; renderImporter(); }
  function impTemplate() {
    var csv = GPO.Importer.template(impState.type);
    var blob = new Blob([csv], { type: 'text/csv' });
    var a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'gpo_' + impState.type + '_template.csv'; a.click(); URL.revokeObjectURL(a.href);
  }

  // ================================================================
  // MODULE: Original Engine — drive the LOCKED tools on gpo_db data
  // ================================================================
  function renderEngine() {
    var v = $('view-engine');
    var suppliers = GPO.Store.all('suppliers');
    v.innerHTML =
      '<h2 class="page">Original Tools Engine</h2>' +
      '<p class="page-sub">Runs the <b>locked</b> Order Generator and PRO-TRACK functions live on your cloud data — the shell builds the inputs and calls the tools\' own code (unmodified).</p>' +
      '<div class="card"><h3>🧾 Order Generator — constrained order calc</h3>' +
        '<div class="row">' +
          field('Suppliers', '<select id="enSup" multiple size="4" style="min-height:auto">' + suppliers.map(function (s) { return '<option value="' + s.id + '" selected>' + esc(s.name) + '</option>'; }).join('') + '</select>') +
          field('Stock cover', '<select id="enCover">' + ['30', '45', '60', '75'].map(function (x) { return '<option value="' + x + '"' + (x === '45' ? ' selected' : '') + '>' + x + ' days</option>'; }).join('') + '</select>') +
          field('Days / month', '<input id="enDpm" type="number" value="30">') +
          field('Layout', '<select id="enMode"><option value="consolidated">Consolidated</option><option value="perBranch">Per-branch</option></select>') +
        '</div>' +
        '<div style="margin-top:12px"><button class="act" id="enRun">▶ Run engine</button> ' +
          '<button class="act ghost" id="enXlsx">⬇ Download order sheet (.xlsx)</button></div>' +
        '<div id="enOut" style="margin-top:14px"></div>' +
        (suppliers.length ? '' : '<div class="empty"><span class="big">⚙️</span>No suppliers yet — import data first (Data Importer) or seed demo data.</div>') +
      '</div>' +
      '<div class="card"><h3>🏭 PRO-TRACK — payment maturity schedule</h3>' +
        '<p class="muted">Calls PRO-TRACK\'s own <code>calculateMaturityDate()</code> over your imported purchase orders.</p>' +
        '<button class="act ghost" id="enPay">▶ Compute payment schedule</button>' +
        '<div id="enPayOut" style="margin-top:12px"></div></div>';

    function selectedSuppliers() { return Array.prototype.map.call($('enSup').selectedOptions, function (o) { return o.value; }); }
    function engineOpts() { return { coverDays: +$('enCover').value, daysPerMonth: +$('enDpm').value || 30, outputMode: $('enMode').value }; }

    if ($('enRun')) $('enRun').addEventListener('click', function () {
      var ids = selectedSuppliers(); if (!ids.length) { toast('Select at least one supplier'); return; }
      $('enOut').innerHTML = '<span class="muted">Running locked engine…</span>';
      GPO.Engine.calculateOrders(ids, engineOpts()).then(function (runData) {
        var rows = runData.map(function (x) {
          var units = x.calc.results.reduce(function (a, r) { return a + r.totalOrder; }, 0);
          return '<tr><td>' + esc(x.supplier.name) + '</td><td class="num">' + x.calc.results.length + '</td><td class="num">' + x.calc.branches.length + '</td><td class="num">' + x.calc.windowDays + '</td><td class="num">' + units.toLocaleString() + '</td></tr>';
        }).join('');
        $('enOut').innerHTML = '<div class="note">✅ Ran the original <code>calculateOrders()</code> on ' + runData.length + ' supplier(s).</div>' +
          '<div class="tbl-wrap" style="margin-top:10px"><table><thead><tr><th>Supplier</th><th class="num">SKUs</th><th class="num">Branches</th><th class="num">Window (d)</th><th class="num">Units to order</th></tr></thead><tbody>' + rows + '</tbody></table></div>';
      }).catch(function (e) { $('enOut').innerHTML = '<div class="note" style="border-left-color:var(--bad)">⚠ ' + esc(e.message) + '</div>'; });
    });

    if ($('enXlsx')) $('enXlsx').addEventListener('click', function () {
      var ids = selectedSuppliers(); if (!ids.length) { toast('Select at least one supplier'); return; }
      toast('Building order sheet via locked tool…');
      GPO.Engine.downloadOrderWorkbook(ids, engineOpts()).then(function () { toast('Order sheet downloaded'); })
        .catch(function (e) { $('enOut').innerHTML = '<div class="note" style="border-left-color:var(--bad)">⚠ ' + esc(e.message) + '</div>'; });
    });

    if ($('enPay')) $('enPay').addEventListener('click', function () {
      $('enPayOut').innerHTML = '<span class="muted">Calling PRO-TRACK…</span>';
      GPO.Engine.paymentSchedule().then(function (sched) {
        if (!sched.length) { $('enPayOut').innerHTML = '<div class="empty">No purchase orders yet — import some first.</div>'; return; }
        var rows = sched.map(function (s) { return '<tr><td>' + esc(s.poNumber) + '</td><td>' + esc(s.supplier) + '</td><td>' + esc(s.term) + '</td><td>' + esc(s.invoiceDate) + '</td><td><b>' + esc(String(s.dueDate)) + '</b></td><td class="num">' + money(s.value) + '</td></tr>'; }).join('');
        $('enPayOut').innerHTML = '<div class="tbl-wrap"><table><thead><tr><th>PO</th><th>Supplier</th><th>Term</th><th>Invoice</th><th>Due date</th><th class="num">Value</th></tr></thead><tbody>' + rows + '</tbody></table></div>';
      }).catch(function (e) { $('enPayOut').innerHTML = '<div class="note" style="border-left-color:var(--bad)">⚠ ' + esc(e.message) + '</div>'; });
    });
  }

  // ================================================================
  // MODULE: Users & Roles (manager-only) — assign custom-claim roles
  // ================================================================
  function renderUsers() {
    var v = $('view-users');
    if (!GPO.RBAC.can('manageRoles')) {
      v.innerHTML = '<h2 class="page">Users &amp; Roles</h2><div class="card"><div class="empty"><span class="big">🔒</span>Only a Procurement Manager can manage roles.</div></div>';
      return;
    }
    var roleOpts = GPO.RBAC.ROLES.map(function (r) { return opt(r, GPO.RBAC.label(r) + ' (' + r + ')', ''); }).join('');
    v.innerHTML =
      '<h2 class="page">Users &amp; Roles</h2>' +
      '<p class="page-sub">Assign a role to a user by email. The role is written as a Firebase Auth <b>custom claim</b> and enforced by Firestore Security Rules. The user must sign out/in (or wait for token refresh) to pick up a new role.</p>' +
      '<div class="card"><h3>👤 Assign role</h3><div class="row">' +
        field('User email', '<input id="urEmail" type="email" placeholder="person@company.com">') +
        field('Role', '<select id="urRole">' + roleOpts + '</select>') +
        '<div class="f" style="flex:0"><label>&nbsp;</label><button class="act" id="urAssign">Assign role</button></div>' +
      '</div><div id="urOut" style="margin-top:12px"></div></div>' +
      '<div class="card"><h3>Role permissions matrix</h3><div class="tbl-wrap">' + rolesMatrixTable() + '</div></div>' +
      '<div class="note">Assigning requires the deployed <code>setUserRole</code> Cloud Function (manager-only, verified server-side). ' +
        'No Functions deployment yet? Bootstrap the first manager with <code>node admin/set-role.js you@email.com manager</code> (see README).</div>';

    $('urAssign').addEventListener('click', function () {
      var email = $('urEmail').value.trim(), role = $('urRole').value;
      if (!email) { toast('Enter an email'); return; }
      var out = $('urOut'); out.innerHTML = '<span class="muted">Assigning…</span>';
      if (!(window.firebase && firebase.functions)) { out.innerHTML = '<div class="note" style="border-left-color:var(--bad)">Cloud Functions SDK not available.</div>'; return; }
      try {
        var call = firebase.functions().httpsCallable('setUserRole');
        call({ email: email, role: role }).then(function (res) {
          out.innerHTML = '<div class="note">✅ ' + esc((res.data && res.data.message) || ('Role ' + role + ' assigned to ' + email)) + '</div>';
          toast('Role assigned');
        }).catch(function (e) {
          out.innerHTML = '<div class="note" style="border-left-color:var(--bad)">⚠ ' + esc(e.message || String(e)) +
            '<br><span class="muted">If the function isn\'t deployed yet, use the admin script (see README).</span></div>';
        });
      } catch (e) { out.innerHTML = '<div class="note" style="border-left-color:var(--bad)">⚠ ' + esc(e.message) + '</div>'; }
    });
  }
  function rolesMatrixTable() {
    var cols = Object.keys(GPO.RBAC.WRITE);
    var head = '<tr><th>Role</th>' + cols.map(function (c) { return '<th>' + c + '</th>'; }).join('') + '<th>manage roles</th></tr>';
    var body = GPO.RBAC.ROLES.map(function (r) {
      var cells = cols.map(function (c) { return '<td style="text-align:center">' + (GPO.RBAC.WRITE[c].indexOf(r) >= 0 ? '✅' : '·') + '</td>'; }).join('');
      var admin = '<td style="text-align:center">' + (GPO.RBAC.CAP.manageRoles.indexOf(r) >= 0 ? '✅' : '·') + '</td>';
      return '<tr><td><b>' + esc(GPO.RBAC.label(r)) + '</b><br><span class="muted">' + r + '</span></td>' + cells + admin + '</tr>';
    }).join('');
    return '<table><thead>' + head + '</thead><tbody>' + body + '</tbody></table><p class="muted" style="margin-top:8px">All roles have full read access. Cells show <b>write</b> permission per collection (mirrors firestore.rules).</p>';
  }
  ...
  function renderAuthArea() {
    ...
    if (u) {
      ...
      var role = GPO.RBAC.role();
      var roleChip = role ? '<span class="pill info" style="margin-right:8px">' + esc(GPO.RBAC.label(role)) + '</span>'
        : '<span class="pill warn" style="margin-right:8px">No role</span>';
      el.innerHTML = roleChip +
        '<div class="user-chip"><span class="name">' + esc(u.displayName || u.email) + '</span><span class="avatar">' + avatar + '</span></div>' +
  ...
  // Show/hide administration-only nav (manager) and role-gated write buttons.
  function applyRoleUi() {
    var isAdmin = GPO.RBAC.can('manageRoles');
    document.querySelectorAll('[data-admin]').forEach(function (eln) { eln.hidden = !isAdmin; });
    var seedBtn = document.querySelector('[onclick*="seed"]');
    // Top-bar seed/export gating (buttons still exist; disable when not allowed)
    document.querySelectorAll('.topbar .act').forEach(function (b) {
      var t = b.textContent;
      if (/Seed/.test(t)) b.disabled = !GPO.RBAC.can('seedDemo');
      if (/Export/.test(t)) b.disabled = !GPO.RBAC.can('exportData');
    });
  }

  // Full-screen "awaiting role" state when signed in but no role claim yet.
  function showAwaiting(show) {
    var id = 'awaitingScreen', ex = $(id);
    if (!show) { if (ex) ex.remove(); return; }
    if (ex) return;
    var d = document.createElement('div');
    d.id = id; d.className = 'login-screen';
    d.innerHTML = '<div class="login-card"><span class="logo lg">GP</span>' +
      '<h2>Awaiting access</h2><p>Your account <b>' + esc((GPO.Auth.currentUser() || {}).email || '') + '</b> is signed in but has no role yet. ' +
      'A Procurement Manager must assign one before you can access the data.</p>' +
      '<button class="act ghost" onclick="GPO.Auth.refreshClaims()">↻ I\'ve been assigned — refresh</button>' +
      '<p class="login-note">Managers assign roles under Administration → Users &amp; Roles.</p>' +
      '<div style="margin-top:14px"><button class="act sm" onclick="GPO.Auth.signOut()">Sign out</button></div></div>';
  ...
  function startApp() {
    ...
    GPO.Store.init().then(function () { setModeBadge(); applyRoleUi(); route(currentRoute || 'dashboard'); })
  ...
  function init() {
    ...
    // Blocked-write feedback (RBAC): show a toast when a role lacks permission.
    if (GPO.RBAC) GPO.RBAC.onDeny(function (collection) { toast('🔒 Your role can’t modify ' + collection); });

    // NO-AUTH MODE: open straight to the app, no sign-in, always manager.
    if (window.GPO_NO_AUTH) {
      showLogin(false); showAwaiting(false); applyRoleUi();
      startApp();
      return;
    }
    ...
    renderAuthArea();
    if (!cloud) { showLogin(false); showAwaiting(false); startApp(); return; }

    showLogin(true); // covered until first auth callback resolves
    GPO.Auth.onChange(function (user) {
      renderAuthArea();
      applyRoleUi();
      if (!user) { showLogin(true); showAwaiting(false); _started = false; return; } // signed out
      showLogin(false);
      if (!GPO.RBAC.isKnown()) { showAwaiting(true); }   // signed in, no role yet
      else { showAwaiting(false); startApp(); }
    });
  }
  ...
  return {
    ...
    impSetType: impSetType, impTemplate: impTemplate,
