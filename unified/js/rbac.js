...
GPO.RBAC = (function () {
  ...
  // NO-AUTH MODE (temporary): treat everyone as manager, regardless of sign-in.
  function noAuth() { return !!window.GPO_NO_AUTH; }

  function enforce() {
    if (noAuth()) return false; // no gating in no-auth mode
    return !!(GPO.Auth && GPO.Auth.isCloud());
  }

  function role() {
    if (noAuth()) return 'manager';
