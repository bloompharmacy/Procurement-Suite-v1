...
## Open Testing Mode (temporary)

For a testing phase where you don't want to assign a role to every colleague, flip **two switches** — any Google sign-in then gets full manager access:

1. **Client:** in [`unified/js/firebase-config.js`](unified/js/firebase-config.js), `window.GPO_OPEN_TESTING = true;` (already on). Any signed-in user is treated as manager and the "Awaiting access" screen is bypassed. A red banner shows while it's active.
2. **Server:** Firebase console → Firestore → Rules → paste [`firestore.rules.testing`](firestore.rules.testing) → Publish. This allows any *authenticated* user to read/write (still requires sign-in — not open to anonymous visitors).

Both are required: the client flag alone still hits the strict rules and writes get rejected.

**Revert to strict RBAC when testing ends:**
1. Set `window.GPO_OPEN_TESTING = false;` (or delete the line) and redeploy.
2. Re-publish [`firestore.rules`](firestore.rules) (strict) in the console.
