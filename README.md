...
└── unified/
    ...
    └── js/
        ...
        ├── engine.js                 ← actively drives the locked tools' functions on gpo_db
        ├── importer.js               ← Universal Data Importer (CSV/XLSX → gpo_db)
...
## No-Auth Mode (temporary internal testing)

Zero friction: no sign-in, no prompts, no warnings — the app opens directly to the dashboard and acts as a permanent "manager". Flip **two switches**:

1. **Client:** in [`unified/js/firebase-config.js`](unified/js/firebase-config.js), `window.GPO_NO_AUTH = true;` (already on).
2. **Server:** Firebase console → Firestore → Rules → paste [`firestore.rules.public`](firestore.rules.public) → Publish.

Both are required: the client flag alone still hits the strict rules and writes get rejected.

> ⚠ With the public rules published, **anyone on the internet** who reaches the project can read/write/delete all data — the web config in the client makes the endpoint discoverable, and open Firestore databases are actively scanned by bots. Use only with disposable test data. A zero-friction alternative that keeps the DB closed is **Firebase Anonymous Auth** (silent, no UI) + `if request.auth != null` rules.

**Revert for production:**
1. Set `window.GPO_NO_AUTH = false;` (or delete the line) and redeploy.
2. Re-publish [`firestore.rules`](firestore.rules) (strict RBAC) in the console.
3. Assign real roles via `admin/set-role.js` or the Users & Roles screen.
...
Your app will be live at `https://<you>.github.io/<repo>/`. The root `index.html` redirects into `unified/`, and the iframes reach the two locked tools via relative `../` paths — all same-origin, which is what lets the shell call the tools' functions directly.

## 3. Getting data in & using the locked tools

**Data Importer** (sidebar → Data Importer): upload CSV/XLSX for **suppliers**, **sellout** (per branch/item/month), or **purchase orders**. Columns are auto-mapped (confirm/adjust), then written straight into `gpo_db` so forecasting, margins and the dashboard have real data. Download a CSV template for each type from the module.

**Original Engine** (sidebar → Original Engine): the shell **actively drives the locked tools' own functions** on your cloud data — it does *not* scrape their state (their `let suppliers`/`STATE` aren't reachable cross-frame; only their `function`s are). It builds the exact inputs from `gpo_db` and calls:
- the Order Generator's `calculateOrders()` / `buildWorkbook()` / `downloadWorkbook()` → constrained order quantities and the real `.xlsx` order sheet;
- PRO-TRACK's `calculateMaturityDate()` → a payment-maturity schedule over your imported POs.

The locked files remain byte-for-byte unchanged.
